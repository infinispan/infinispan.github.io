...
Infinispan introduced Cross Site Replication functionality in version 5.2 and Infinispan 7 extends it to
...
Each site has Site Masters; nodes with special roles, and responsible for the communication between sites.

RELAY2 can use more the one Site Master per site allowing load balance the inter-site requests.
...
`max_site_masters` value to a higher than 1.
...
Taking advantage of vector clocks, in the asynchronous mode, Infinispan is able to detect conflicts.
...
Infinispan resolves the conflicts using the site's name lexicographically order.
The site's name lower in lexicographically order takes priority.
